...
def initial_state():
    ...
    # *** START CODE HERE ***
    mydict = {}
    x_0 = np.zeros((1,1)) #incorporates theta_0 into the mix
    alpha_0 = 0
    index = 0
    mydict[index] = (alpha_0, x_0)
    return mydict
    # *** END CODE HERE ***
...
def predict(state, kernel, x_i):
    ...
    """
    # *** START CODE HERE ***
    #x_i is really x_i+1 here
    
    max_index = len(state) - 1 #number of i's
    if max_index == 0: return 1
    argument = 0
    for j in range(max_index + 1):
        if j ==0 : continue
        #print(state[j])
        argument += state[j][0] * kernel(state[j][1], x_i)
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    """
    # *** START CODE HERE ***
    #y_i/x_i actually i+1
    max_index = len(state) - 1 
    alpha_i_plus_one = (y_i - predict(state, kernel, x_i))* learning_rate
    state[max_index + 1] = (alpha_i_plus_one, x_i)
    # *** END CODE HERE ***
